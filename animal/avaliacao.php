...
$dados = mysql_select_db("siste870_sob2",$db);

$media1 = mysql_query("SELECT AVG(tipo) FROM avaliacao WHERE avaliacao = '1'");
while($row = mysql_fetch_assoc($media1)) {
     $media1 = $row['AVG(tipo)'];
}

$media2 = mysql_query("SELECT AVG(tipo) FROM avaliacao WHERE avaliacao = '2'");
while($row = mysql_fetch_assoc($media2)) {
     $media2 = $row['AVG(tipo)'];
}
mysql_close($db);
?>
